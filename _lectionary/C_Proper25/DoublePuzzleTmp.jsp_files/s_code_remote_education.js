...
var network_definitions = new Array(   [ "disccedu", 	"Education",	"Education",	"Discovery Education", 		"www.discoveryeducation.com" ]
	,[ "disccedu",			"Education",	"Education Blog",		"Education Blog", 		"blog.discoveryeducation.com" ]
	,[ "disccedu",			"Education",	"Education Registration",	"Education Registration",	"connect.discoveryeducation.com" ]
	,[ "disccunstream",		"Education",	"United Streaming",		"United Streaming",		"streaming.discoveryeducation.com" ]
	,[ "disccedu",			"Education",	"Education Create",		"Education Create",		"create.discoveryeducation.com" ]
	,[ "disccschool",		"Education",	"Puzzlemaker",			"Puzzlemaker",			"puzzlemaker.discoveryeducation.com" ]
	,[ "disccedu",			"Education",	"Education Reports",		"Education Reports",		"reports.discoveryeducation.com" ]
	,[ "disccedu",			"Education",	"Education Player",		"Education Player", 		"player.discoveryeducation.com" ]
	,[ "disccednetwork",		"Education",	"Discovery Educator Network",	"Dicovery Educator Network", 	"community.discoveryeducation.com" ]
	,[ "disccsciconn",		"Education",	"Science Connection",		"Science Connection",		"science.discoveryeducation.com" ]
	,[ "discchealthcon",		"Education",	"Health Connection",		"Health Connection",		"health.discoveryeducation.com" ]
	,[ "disccedu",			"Education",	"Teacher Store",		"Teacher Store", 		"store.discoveryeducation.com" ]
	,[ "discpowermedia",		"Education",	"Power Media Plus",		"Power Media Plus", 		"powermediaplus.com" ]
	,[ "disccthinklink",		"Education",	"Think Link Learning",		"Think Link Learning", 		"thinklinklearning.com" ]
	,[ "disccthinklink",		"Education",	"Think Link Assessment",	"Think Link Assessment",	"thinklinklearning.com" ]
	,[ "disccschool",		"Education",	"Make a Splash",		"Make a Splash",		"makeasplash.discovery.com" ]
	,[ "disccschool",		"Education",	"School",			"School",			"school.discoveryeducation.com" ]
	,[ "disccedu",			"Education",	"Academy 123",			"Academy 123",			"nutshellmath.com" ]
	,[ "disccedu",			"Education",	"Educator Abroad",		"Educator Abroad",		"discoveryeducatorabroad.com" ]
	,[ "discclearningaddsup",	"Education",	"Learning Adds Up",		"Learning Adds Up",		"learningaddsup.com" ]
	,[ "discclearvue",		"Education",	"ClearVue",			"ClearVue", 			"clearvue.com" ]
);


var excludePages = new Array ( "_npv", "ad_popup", "ad_interstitial", "includes" );
...
var s_linkDownloadFileTypes		= "exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,sit,m4v";
var s_linkInternalFilters		= "javascript:,.";
var s_linkLeaveQueryString		= false;
var s_linkTrackVars			= "s_pageName,s_channel,s_prop1,s_prop2,s_prop3,s_prop4,s_prop5,s_prop6,s_prop7,s_prop8,s_prop9,s_prop10";
var s_linkTrackEvents			= "None";

/* Plugin Config */
var s_usePlugins			= true

//break full path into pieces, grabbing only the first two parts (pkg/loc)
var dirs = dPath.split("/",3);

s_linkInternalFilters = "javascript:";

var rslen = network_definitions.length;
for (var i=0; i<rslen; i++)
{
	var rsuite = network_definitions[i][0];   //get the report suite
      	var dlist = network_definitions[i][4];    //get the domain(s)

	//if ( dlist.indexOf(d) != -1 )
	if ( dHost.indexOf(dlist) != -1 )
	{
		var s_account = rsuite;
		s_linkInternalFilters += "," + dlist;
	}
		
}

//set the Network, Category, Description fields
var arylen = network_definitions.length;
for ( var n=0; n<arylen; n++ )
{
	var curHost = network_definitions[n][4];
	if ( curHost == dHost || ("media." + curHost == dHost) )
	{
		var business_org = network_definitions[n][1];
		var network_type = network_definitions[n][2];
		// not needed for Education:  var content_type = network_definitions[n][3];
	
		dNet = network_type.toLowerCase();

		s_prop1 = business_org;
		s_prop2 = business_org + ":" + network_type;
		// not needed for Education:  s_prop3 = business_org + ":" + network_type + ":" + content_type;
		s_channel = network_type;
	}
}
...
{
	var validPage = true;
	// make sure we're a trackable page
	if ( dProt == "file:" ) validPage = false;
	
	for (var x=0; x<excludePages.length; x++)
	{ 
		if ( dPath.indexOf(excludePages[x]) != -1 ) validPage = false;
	}
...
function s_doPlugins() {
	...
	/* Specify pageName based on URL if it doesn't already exist */
	if (! s_vp_getValue('s_pageName') ) {
		s_vpr('s_pageName', dProt + '//' + dHost + dPath )
	}

	/* Specify s_channel based on URL if it doesn't already exist */
	//	if ( s_channel == "" )
	if (! s_vp_getValue('s_channel') )
	{
		if ( dPath == "/index.cfm" || dPath == "/" )
		{
			if (!s_prop7)
			{
				s_vpr( 's_prop7', network_type + ":" + "homepage" );
			}
		}
	}
	
	//get campaign source code
	var campaign_tc = ( location.search.indexOf("dcitc") != -1 ) ? "dcitc" : "source";
	s_vp_getCGI('s_campaign', campaign_tc);

	//internal link tracking
	var clickthru = ( location.search.indexOf("clik") != -1 ) ? "clik" : "click";
	s_vp_getCGI('s_eVar1', clickthru);

	//cleanup random artifacts
	if ( s_vp_getValue('s_prop4') == "x26" ) { s_vpr('s_prop4', "video" ); }
	if ( s_vp_getValue('s_prop5') == "&vp=1.07" ) { s_vpr('s_prop5', "" ); }
	if ( s_vp_getValue('s_linkName') == "vp107" ) { s_vpr('s_linkName', "" ); }


	//debugging 
	if ( location.search.indexOf("debug") != -1 )
	{
		strHTML += "<div class=creditsText>";
		strHTML += "<br/>pageName = " + s_vp_getValue('s_pageName');
		strHTML += "<br/>dPath = " + dPath;
		strHTML += "<br/>dHost = " + dHost;
		strHTML += "<br/>dProt = " + dProt;
		strHTML += "<br/>dNet = " + dNet;
		strHTML += "<br/>s_val = " + s_val;
		//strHTML += "<br/>rsuite = " + rsuite;
		//strHTML += "<br/>dlist = " + dlist;
		
		strHTML += "<br/>s_prop1 					= " + s_vp_getValue('s_prop1');
		strHTML += "<br/>s_prop2 					= " + s_vp_getValue('s_prop2');
		strHTML += "<br/>s_prop3 					= " + s_vp_getValue('s_prop3');
		strHTML += "<br/>s_prop4 					= " + s_vp_getValue('s_prop4');
		strHTML += "<br/>s_prop5 					= " + s_vp_getValue('s_prop5');
		strHTML += "<br/>s_prop6 					= " + s_vp_getValue('s_prop6');
		strHTML += "<br/>s_prop7 					= " + s_vp_getValue('s_prop7');
		strHTML += "<br/>s_prop8 					= " + s_vp_getValue('s_prop8');
		strHTML += "<br/>s_prop9 					= " + s_vp_getValue('s_prop9');
		strHTML += "<br/>s_prop10					= " + s_vp_getValue('s_prop10');

		strHTML += "<br/>s_eVar1					= " + s_vp_getValue('s_eVar1');

		strHTML += "<br/>s_campaign					= " + s_vp_getValue('s_campaign');
		strHTML += "<br/>s_channel					= " + s_vp_getValue('s_channel'); 
		strHTML += "<br/>s_account 					= " + s_account;
		strHTML += "<br/>s_linkInternalFilters		= " + s_linkInternalFilters;
		strHTML += "</pre><br/></div>";
	}	//end debug
...
vh.substring(ti,pi<0?vh.length:pi);return t}
function s_vh_gl(k){var
vh=s_c_r('s_vh_'+k),e=vh?vh.indexOf('='):0;return vh?(vh.substring(0,
e?e:vh.length)):''}
function s_vh_s(k,v){if(k&&v){var e=new Date,st=
...
parseFloat(s_v);
function s_fl(s,l){return s?(s+'').substring(0,l):s}
...
return 0}return 1}
function s_rep(s,o,n){var i=s.indexOf(o),l=n.length>
0?n.length:1;while(s&&i>=0){s=s.substring(0,i)+n+s.substring(i
+o.length);i=s.indexOf(o,i+l)}return s}
function s_ape(s){return s?
s_rep(escape(''+s),'+','%2B'):s}
function s_epa(s){return s?unescape(
s_rep(''+s,'+',' ')):s}
function s_pt(s,d,f,a){var t=s,x=0,y,r;while(t)
...
return ''}
function s_isf(t,a){if(t.substring(0,2)=='s_')t=t.substring(
2);return (t!=''&&t==a)}
function s_fsf(t,a){if(s_pt(a,',',s_isf,t))
s_fsg+=(s_fsg!=''?',':'')+t;return 0}var s_fsg;
function s_fs(s,f){
s_fsg='';s_pt(s,',',s_fsf,f);return s_fsg}var s_c_d=''
function s_c_gdf(t,a){if(!s_num(t))return 1;return 0}
function s_c_gd()
...
d.substring(p):''}return s_c_d}
function s_c_r(k){k=s_ape(k);var c=' '
...
' domain='+d+';':'');return s_c_r(k)==v}return 0}
function s_cet(f,a,
et,oe,fb){var r,d=0
...
return s_tfs}
function s_gtfs(){if(!s_tfs){s_tfs=s_wd;if(!s_etfs)s_tfs=
...
function s_it(un){s_ca(un)}
function s_mr(un,sess,q,ta){un=
...
)+'://'+(s_ssl?'simg':'img')+'.discovery.com/b/ss/'+un+'/1/G.9-Pd-R/'
...
function s_lnf(t,h){t=t?t.toLowerCase():'';h=h?h.toLowerCase():'';var te=t.indexOf('=');if(t&&
te>0&&h.indexOf(t.substring(te+1))>=0)return t.substring(0,te)
return ''}
function s_ln(h){if (s_gg('linkNames'))return s_pt(s_gg(
'linkNames'),',',s_lnf,h);return ''}
function s_ltdf(t,h){t=t?
t.toLowerCase():'';h=h?h.toLowerCase():'';var qi=h.indexOf('?');h=qi>=
0?h.substring(0,qi):h;if(t&&h.substring(h.length-(t.length+1))=='.'+t)
return 1;return 0}
function s_ltef(t,h){t=t?t.toLowerCase():'';h=h?
...
);o.s_oidt=x}}return o.s_oid}
function s_rqf(t,un){var e=t.indexOf('='
),u=e>=0?','+t.substring(0,e)+',':'';return u&&u.indexOf(','+un+',')>=
0?s_epa(t.substring(e+1)):''}
function s_rq(un){var c=un.indexOf(','),
v=s_c_r('s_sq'),q='';if(c<0)return s_pt(v,'&',s_rqf,un);return s_pt(
un,',',s_rq,0)}var s_sqq,s_squ;
function s_sqp(t,a){var e=t.indexOf('='
),q=e<0?'':s_epa(t.substring(e+1));s_sqq[q]='';if(e>=0)s_pt(
t.substring(0,e),',',s_sqs,q);return 0}
function s_sqs(un,q){s_squ[un]=
q;return 0}
function s_sq(un,q){s_sqq=new Object;s_squ=new Object
...
+s_sqq[x]+'='+s_ape(x);c++}return s_c_w(k,v,0)}
function s_wdl(e){
s_wd.s_wd_l=1;var r=true;if(s_wd.s_ol)r=s_wd.s_ol(e);if(s_wd.s_ls)
s_wd.s_ls();return r}
function s_wds(un){un=un.toLowerCase()
...
s_wd.onload;s_wd.onload=s_wdl}s_wd.s_olr=1}}}
function s_iepf(i,a){if(
...
function s_gs(un){un=un.toLowerCase();s_un=un;
var trk=1,tm=new Date,sed=Math&&Math.random?Math.floor(
...
s_sq(un,trk?'':qs)}return code}
function s_dc(un){un=un.toLowerCase()
s_wds(un);s_ca(un);return s_gs(un)}
s_val=isValid(); if(s_val) {  s_code=s_dc(s_account); if(s_code)s_d.write(s_code); } 
